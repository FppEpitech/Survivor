  ...
  constructor(private eventService: EventsService, private _tloco: TranslocoService, public _auth: AuthService) {
    this._tloco.langChanges$.subscribe(lang => {
      ...
      this.calendarOptions.buttonText = {
        month: this._tloco.translate('monthTxtEv') == 'monthTxtEv' ? 'Month' : this._tloco.translate('monthTxtEv'),
        week: this._tloco.translate('weekTxtEv') == 'weekTxtEv' ? 'Week' : this._tloco.translate('weekTxtEv'),
        day: this._tloco.translate('dayTxtEv') == 'dayTxtEv' ? 'Day' : this._tloco.translate('dayTxtEv'),
        list: this._tloco.translate('listTxtEv') == 'listTxtEv' ? 'List' : this._tloco.translate('listTxtEv')
      }
    });
  }
    ...
    this.eventService.getEvents().subscribe(events => {
      ...
      for (const event of events) {
        ...
        L.marker([parseFloat(event.location_x), parseFloat(event.location_y)]).addTo(this.map!)
          .bindPopup(`${this._tloco.translate('maxParticipantsTxt')}: ${event.max_participants}, ${this._tloco.translate('dateTxt')}: ${event.date} --> : ${event.location_name}`)
          .openPopup();
      }
    });
  }
